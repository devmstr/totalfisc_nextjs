    ...
    const result = await prisma.$transaction(async (tx) => {
      const piece = await tx.piece.create({
        data: {
          ...
          tenantId: tenantId, // Now required and supported
        },
      });
...
export async function updatePiece(pieceId: string, data: z.infer<typeof createPieceSchema>) {
  try {
    const session = await getServerSession(authOptions);
    ...
    if (!balanceCheck.isValid) {
      return { success: false, error: `Unbalanced: ${balanceCheck.difference.toFixed(2)}` };
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const line of validated.lines) {
        ...
        if (account.isAuxiliaryRequired && !line.auxiliaryId) throw new Error(`Auxiliary required for ${line.accountCode}`);
